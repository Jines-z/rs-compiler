...
function HttpProxyMiddleware(context, opts) {
  ...
  function handleUpgrade(req, socket, head) {
    ...
    if (shouldProxy(config.context, req)) {
      ...
      logger.info(`  ${chalk.gray('｢hpm｣')}: Upgrading to WebSocket`)
    }
  }
  ...
  function __applyRouter(req, options) {
    ...
    if (options.router) {
      ...
      if (newTarget) {
        logger.debug(
          `  ${chalk.gray('｢hpm｣')}: Router new target: %s -> "%s"`,
          options.target,
          newTarget
        )
  ...
  function __applyPathRewrite(req, pathRewriter) {
    if (pathRewriter) {
      ...
      if (typeof path === 'string') {
        ...
        logger.info(`  ${chalk.gray('｢hpm｣')}: pathRewrite: No rewritten path found. (%s)`, req.url)
      }
    }
  }
  ...
  function logError(err, req, res) {
    ...
    var errorMessage =
      `  ${chalk.gray('｢hpm｣')}: Error occurred while trying to proxy request %s from %s to %s (%s) (%s)`
    var errReference =
